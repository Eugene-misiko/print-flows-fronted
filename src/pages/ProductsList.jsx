...
import {fetchProducts, createProduct, updateProduct, deleteProduct,fetchCategories, createCategory,} from "@/store/slices/productsSlice";
...
/* Compact dynamic field renderer */
const FieldInput = ({ field, value, onChange }) => {
  ...
  if (field.field_type === "checkbox") return (
    <label className="flex items-center gap-2 cursor-pointer">
      <input type="checkbox" checked={value || false} onChange={(e) => onChange(e.target.checked)} className="w-4 h-4 text-orange-600 rounded focus:ring-orange-500 dark:focus:ring-orange-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
      <span className="text-sm text-gray-600 dark:text-gray-400">{field.help_text || field.name}</span>
    </label>
  );
  if (field.field_type === "file") return (
    <div>
      <input type="file" onChange={(e) => e.target.files[0] && onChange(e.target.files[0].name)}
        className="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-orange-50 dark:file:bg-orange-900/30 file:text-orange-700 dark:file:text-orange-400 hover:file:bg-orange-100 cursor-pointer" />
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Browse and add products to your order</p>
        </div>
        <div className="flex gap-2">
          ...
          {isAdmin && (
            <>
              <button onClick={() => setShowCategoryModal(true)} className="flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
        ...
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium text-green-800 dark:text-green-300">{cart.length} item{cart.length > 1 ? "s" : ""} — KES {cartTotal.toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => navigate("/orders/new")} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium">Checkout</button>
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {cart.map((item) => (
              <span key={item.cartId} className="inline-flex items-center gap-1 px-2 py-1 bg-white dark:bg-gray-800 rounded-lg text-xs border border-green-200 dark:border-green-700 text-gray-700 dark:text-gray-300">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h2 className="font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2"><Folder className="w-5 h-5 text-orange-500" /> Categories</h2>
        <div className="flex flex-wrap gap-2">
          {categories?.length === 0 ? <p className="text-gray-500 dark:text-gray-400 text-sm">No categories yet</p> : (
            categories.map((cat) => (
              <span key={cat.id} onClick={() => dispatch(fetchProducts({ category: cat.id }))}
                className="cursor-pointer px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-full text-sm hover:bg-orange-100 dark:hover:bg-orange-900/30 transition-colors">
                {cat.name}
              </span>
            ))
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          return (
            <div key={product.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-lg transition-all flex flex-col">
              <div className="relative h-44 w-full overflow-hidden">
                <img src={product.image || "https://via.placeholder.com/400x300"} alt={product.name} className="w-full h-full object-cover" />
                <span className="absolute top-3 right-3 text-xs px-3 py-1 rounded-full bg-white/90 dark:bg-gray-900/80 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-600">{product.category_name || "General"}</span>
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex-1 line-clamp-2">{product.description || "No description"}</p>
                {product.fields?.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {product.fields.slice(0, 3).map((f) => <span key={f.id} className="text-xs px-2 py-0.5 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 rounded">{f.name}</span>)}
                    {product.fields.length > 3 && <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-500 rounded">+{product.fields.length - 3}</span>}
                  </div>
                )}
                <p className="text-xs text-gray-400 mt-2">Min: {product.min_quantity || 1} · Max: {product.max_quantity || 10000}</p>
                ...
                {isAdmin && (
                  <div className="flex justify-end gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                    <button onClick={() => openEdit(product)} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                      <Edit className="w-4 h-4 text-gray-500" />
                      </button>
                    <button onClick={() => handleDelete(product.id)} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition">
            ...
            <div className="p-6 pb-0">
              ...
              <div className="flex gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-600 dark:text-green-400 font-semibold">KES {selectedProduct.price?.toLocaleString()}</p>
                </div>
              </div>
            </div>
            <div className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Quantity * <span className="text-xs text-gray-400">(Min: {selectedProduct.min_quantity || 1}, Max: {selectedProduct.max_quantity || 10000})</span>
                </label>
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setItemForm((p) => ({ ...p, quantity: Math.max(selectedProduct.min_quantity || 1, p.quantity - 1) }))}
                    disabled={itemForm.quantity <= (selectedProduct.min_quantity || 1)} className="p-2 rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40"><Minus className="w-4 h-4" /></button>
                  <input type="number" value={itemForm.quantity}
                    ...
                    className="w-24 text-center px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white font-medium outline-none focus:ring-2 focus:ring-orange-500" />
                  <button type="button" onClick={() => setItemForm((p) => ({ ...p, quantity: Math.min(selectedProduct.max_quantity || 10000, p.quantity + 1) }))}
                    disabled={itemForm.quantity >= (selectedProduct.max_quantity || 10000)} className="p-2 rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40"><Plus className="w-4 h-4" /></button>
                  <div className="flex gap-1 ml-2">
                    {[10, 50, 100, 500].map((q) => (
                      <button key={q} type="button" onClick={() => setItemForm((p) => ({ ...p, quantity: Math.min(selectedProduct.max_quantity || 10000, q) }))}
                        className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded hover:bg-orange-100 dark:hover:bg-orange-900/30 hover:text-orange-600">
                          {q}</button>
                    ))}
                  </div>
                </div>
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Subtotal: 
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {selectedProduct.fields.map((field) => (
                      <div key={field.id}>
                        <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">{field.name}{field.required && <span className="text-red-500 ml-1">*</span>}</label>
                        ...
                        {field.help_text && field.field_type !== "checkbox" && <p className="text-xs text-gray-400 mt-1">{field.help_text}</p>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Notes */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes / Special Instructions</label>
              ...
              {selectedProduct.requires_design && (
                <div className="p-3 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
                  <p className="text-sm text-purple-700 dark:text-purple-300"><strong>Design Required:</strong> Provide details during checkout.</p>
                </div>
              )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md border border-gray-200 dark:border-gray-700 p-6">
            ...
            <form onSubmit={handleProductSubmit} className="space-y-4" encType="multipart/form-data">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setShowCategoryModal(true)} className="px-3 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300">+</button>
              </div>
              ...
              <input type="file" accept="image/*" onChange={(e) => e.target.files[0] && setProductForm({ ...productForm, image: e.target.files[0] })}
                className="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-orange-50 dark:file:bg-orange-900/30 file:text-orange-700 dark:file:text-orange-400 hover:file:bg-orange-100 cursor-pointer" />
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => { setShowProductModal(false); setEditingProduct(null); setProductForm({ name: "", category: "", price: "", description: "", image: null }); }} className="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg shadow-sm">{editingProduct ? "Update" : "Create"}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md border border-gray-200 dark:border-gray-700 p-6">
            ...
            <form onSubmit={handleCategorySubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowCategoryModal(false)} className="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg shadow-sm">Create</button>
              </div>
            </form>
          </div>
        </div>
