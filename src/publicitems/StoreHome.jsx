...
const injectStyles = () => {
  ...
  s.id = id;
  s.textContent = `
    @keyframes sh-float-a{0%,100%{transform:translate(0,0) rotate(0deg)}33%{transform:translate(30px,-20px) rotate(120deg)}66%{transform:translate(-15px,15px) rotate(240deg)}}
    ...
    .sh-shimmer-bg{background:linear-gradient(90deg,transparent 25%,rgba(255,255,255,.07) 50%,transparent 75%);background-size:200% 100%;animation:sh-shimmer 2.8s infinite}
    .sh-img-ph{background:linear-gradient(135deg,#e7e5e4 0%,#d6d3d1 50%,#e7e5e4 100%);background-size:200% 200%;animation:sh-shimmer 2.5s infinite}
    .dark .sh-img-ph{background:linear-gradient(135deg,#292524 0%,#1c1917 50%,#292524 100%);background-size:200% 200%;animation:sh-shimmer 2.5s infinite}
    .sh-scrollbar-hide::-webkit-scrollbar{display:none}
    .sh-scrollbar-hide{-ms-overflow-style:none;scrollbar-width:none}
...
const StoreHome = () => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const el = !activeCategory
        ? btns[0]
        : [...btns].find((b) => b.dataset.cid === String(activeCategory));
  ...
  useEffect(() => {
    const h = () => setShowBTT(window.scrollY > 500);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8 pb-16">
          ...
          {!loading && categories.length > 0 && (
            <div className="sh-su sticky top-16 z-40 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 bg-[#faf9f6]/90 dark:bg-stone-950/90 backdrop-blur-xl" style={{ animationDelay: ".05s" }}>
              <div
                ref={catBarRef}
                className="relative flex items-center gap-1.5 overflow-x-auto sh-scrollbar-hide bg-white dark:bg-stone-900 rounded-2xl p-1.5 shadow-sm shadow-stone-200/50 dark:shadow-black/20 border border-stone-200/60 dark:border-stone-700/50"
              >
                <div className="sh-cat-pill absolute top-1.5 bottom-1.5 bg-[#1c1917] dark:bg-white rounded-xl z-0" style={pill} />
                <button
                  data-cb data-cid="all"
                  onClick={() => setActiveCategory(null)}
                  className={`relative z-10 shrink-0 px-5 py-2.5 rounded-xl text-[13px] font-bold transition-colors duration-200 ${!activeCategory ? "text-white dark:text-stone-900" : "text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-200"}`}
                >
                  All Products
                </button>
                {categories.map((c) => (
                  <button
                    key={c.id}
                    data-cb data-cid={c.id}
                    onClick={() => setActiveCategory(c.id)}
                    className={`relative z-10 shrink-0 px-5 py-2.5 rounded-xl text-[13px] font-bold flex items-center gap-1.5 transition-colors duration-200 ${activeCategory === c.id ? "text-white dark:text-stone-900" : "text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-200"}`}
                  >
            ...
            {!loading && filtered.length === 0 && (
              <div className="sh-su text-center py-24">
                <div className="inline-flex flex-col items-center gap-4">
                  <div className="w-20 h-20 rounded-3xl bg-stone-50 dark:bg-stone-800 border border-stone-200/60 dark:border-stone-700 flex items-center justify-center">
                    <Package className="w-8 h-8 text-stone-300 dark:text-stone-600" />
                  </div>
                  <div>
                    <p className="text-stone-700 dark:text-stone-300 font-bold text-base">No products found</p>
                    <p className="text-stone-400 dark:text-stone-500 text-sm mt-1.5">
                      {debouncedSearch ? `No results for "${debouncedSearch}"` : "This category has no products yet"}
                    </p>
                  </div>
